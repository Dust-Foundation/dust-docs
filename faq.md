...
Swaps inside the app are USDG swaps, so those round-ups come from USDG. For capturing outside activity, Capture Everywhere can be funded from USDG today, and from WETH or a small ETH float you park in the Capture contract; those two paths are built and deploying (status: deploying). The [custody page](wallets-and-custody.md) has the details. On Solana, round-ups can be funded from SOL as well as USDC.
