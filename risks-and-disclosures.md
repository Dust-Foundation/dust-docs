...
**Smart contract risk.** Dust's contracts, and the contracts they compose with (the Uniswap v3 pools, USDG, and the Stock Token contracts), can contain bugs. On Solana the same applies to Dust's program, Jupiter, the token program, and Squads. The Dust code is new. The Robinhood Chain contracts were audited by CredShields in September 2026, with every finding fixed and re-verified ([full report](https://github.com/Credshields/audit-reports/blob/master/Audit_Report_Dust.pdf)); the Solana program has not been audited yet, and stays pre-public until it is. Audits, invariant testing, and a bug bounty reduce the odds; nothing makes them zero. Our scoped-permission design limits how much a single failure can reach, which is described honestly in [Security](security.md).
